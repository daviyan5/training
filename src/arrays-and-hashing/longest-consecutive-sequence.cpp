...
/*
    ...
    The problem is: How can we update all the other "participants" in the sequence without falling into a
    O(n²) situation?
    The answer is that we don't really need to. Since we want to know the largest sequence, 
    we only need to update the extremities of our current sequence, since any further contribution to 
    our answer can only come from sequences larger than the current one.
*/
